...
Vsasr::Vsasr(int servo_num):
	servoNum(0),
	numDevice(0),
	dev()
{
	initialize(servo_num);
}
...
void Vsasr::setTargetPosition(double tx, double ty, double tz, double tyaw,
		double tw, unsigned short ttime_ms) {
	...
	//得られた角度にアームを動かす
	int move_ret = 0;
	move_ret = RSMove(dev, sPos, ttime_ms / 10, 1, servoNum);
	// 移動が完了するまでSleep
//	usleep(ttime_ms * 1000);
}

short Vsasr::getAngle(unsigned char servo_id) {

	if (servo_id < 1 || servo_id > 5) {
		printf("Servo id %d is invalid.", (int) servo_id);
		return 0;
	}

	short angle = 0;
	int success = 0;
	success = RSGetAngle(dev, servo_id, &angle);
	if (!success) {
		printf("Faild to get angle of servo %d.\n", servo_id);
		return 0;
	}
...
void Vsasr::getPosition(double &x, double &y, double &z, double &yaw,
		double &width) {

//	double x,y,z,yaw,w;				//現在のアームの座標を表す変数
	short pos[5] = { 0, 0, 0, 0, 0 };		//現在位置を代入する配列変数

	//現在接続されたモータ数だけ現在位置を取得
	for (int i = 0; i < servoNum; i++) {
		int success = 0;
		success = RSGetAngle(dev, i + 1, &pos[i]);
		if (!success) {
...
//	printf("X:%+5d, Y:%+5d, Z:%+5d\n", pos[0], pos[1], pos[2]);
	...
	// モータの回転量を位置に変換
	rad_to_pos(&x, &y, &z, &yaw, &width, pos, 5);
...
int Vsasr::getServoNum(){
	return servoNum;
}

void Vsasr::setGain(bool on) {
	if (on) {
		//ゲインをON
...
void Vsasr::initialize(int servo_num) {
	// サーボ数3,5以外を設定されたらプログラム終了
	if (!(servo_num == 3 || servo_num == 5)) {
		printf("Servo_num must be 3 or 5, but you set %d.\n", servo_num);
		exit(0);
	}
 ...
 *************************************************/
int Vsasr::RSTorqueOnOff(HID_UART_DEVICE dev, short sMode, BYTE id, int num) {
 ...
 *************************************************/
int Vsasr::RSGetAngle(HID_UART_DEVICE dev, BYTE id, short *getParam) {
 ...
 *************************************************/
int Vsasr::RSMove(HID_UART_DEVICE dev, short *sPoss, unsigned short sTime,
		BYTE id, int num) {
 ...
 *************************************************/
int Vsasr::ReadLocalEcho(HID_UART_DEVICE dev, unsigned char *sendbuf,
 ...
 *************************************************/
int Vsasr::RSWriteMem(HID_UART_DEVICE dev, BYTE address, BYTE size, BYTE id,
		BYTE *data, int num) {
	unsigned char sendbuf[256], *bufp;
	unsigned char sum;
	unsigned char i, j;
 ...
 *************************************************/
void Vsasr::rad_to_pos(double *x, double *y, double *z, double *yaw, double *w,
		short *sPos, int num) {
	double tx, ty;
	double lx, ly;

	//ID1,ID2のモータの現在位置より、順運動学でアームの座標を求める
	ty = sin(SVPOS_TO_RAD(sPos[0])) * AXISLEN_A
			+ sin(SVPOS_TO_RAD(sPos[0] + sPos[1])) * AXISLEN_B;
	tx = cos(SVPOS_TO_RAD(sPos[0])) * AXISLEN_A
			+ cos(SVPOS_TO_RAD(sPos[0] + sPos[1])) * AXISLEN_B;
	...
	tx = (lx * cos(BASE_ANGLE / 180.0 * M_PI)
			- ly * sin(BASE_ANGLE / 180.0 * M_PI)) + BASE_OFFSET_X;
	ty = (lx * sin(BASE_ANGLE / 180.0 * M_PI)
			+ ly * cos(BASE_ANGLE / 180.0 * M_PI)) + BASE_OFFSET_Y;
 ...
 *************************************************/
void Vsasr::pos_to_rad(double x, double y, double z, double yaw, double w,
		short *sPos, int sign, int num) {
	...
	lx = x - BASE_OFFSET_X;
	ly = y - BASE_OFFSET_Y;
	x = (lx * cos(-BASE_ANGLE / 180.0 * M_PI)
			- ly * sin(-BASE_ANGLE / 180.0 * M_PI));
	y = (lx * sin(-BASE_ANGLE / 180.0 * M_PI)
			+ ly * cos(-BASE_ANGLE / 180.0 * M_PI));
	...
	if (hypot(tx, ty) >= AXISLEN_A + AXISLEN_B) {
		...
		//余弦定理を演算
		a = acos(
				(-(tx * tx + ty * ty) + AXISLEN_A * AXISLEN_A
						+ AXISLEN_B * AXISLEN_B) / (2 * AXISLEN_A * AXISLEN_B));
		b = s
				* acos(
						(- AXISLEN_A * AXISLEN_A + AXISLEN_B * AXISLEN_B
								+ (tx * tx + ty * ty))
								/ (2 * AXISLEN_A * sqrt(tx * tx + ty * ty)));
 ...
 *************************************************/
int Vsasr::SetTXOpenDrain(HID_UART_DEVICE dev) {
	...
	//現在の Pin Config を取得
	HidUart_GetPinConfig(dev, pinConfig, &useSuspendValues, &suspendValue,
			&suspendMode, &rs485Level, &clkDiv);

	//TX を TX-Open-Drain(0x01) に変更
	pinConfig[10] = 0x01;

	//Pin Config を更新
	return HidUart_SetPinConfig(dev, pinConfig, useSuspendValues, suspendValue,
			suspendMode, rs485Level, clkDiv);
